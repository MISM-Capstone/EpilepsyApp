...
export default class LogSeizure extends React.Component<Props, StateTypes> {
    ...
    async insertQuery(date:Date,location:string,notes:string) {
        let insert = await db.executeSql('INSERT INTO seizure_events (date, location, notes) VALUES (?,?,?)',[date,location,notes]);
        console.log(insert);
    }
    ...
    render() {
        ...
        return (
            <SafeAreaView>
                ...
                <Button title="Save" onPress={() => this.insertQuery(date,locationText,detailsText)} />
